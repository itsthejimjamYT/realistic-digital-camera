...
// GENERATED from 26.2's shaders/include/dof.glsl for the 1.20.4 branch — keep the two in step.
...
// block (1.20.4's EffectInstance has no uniform buffers), and standard depth (near 0, far 1)
// — 1.20.4's vanilla depth buffer uses the same convention a shader pack's does.
//
// This variant has MainDepthSampler fed straight from Iris's depth texture by
// LevelPostMixin (EffectInstance.setSampler), since a shader pack doesn't fill vanilla's.

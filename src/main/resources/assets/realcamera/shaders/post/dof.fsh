...
// 26.3: #moj_import only works for "core" shaders now, not post-processing ones (its
// preprocessing pass never runs for anything compiled through the new SPIR-V pipeline —
// confirmed empirically: the raw "#moj_import" line reached the real GLSL compiler
// unsubstituted and failed to parse). So the shared DoF gather from
// shaders/include/dof.glsl is inlined here by hand instead of imported. Keep this file,
// dof_shaderpack.fsh, and dof.glsl in sync manually when editing the gather logic.
...
// it switches on.
//
// PHOTOMODE_SHADERPACK_DEPTH is NOT defined here (reversed-Z, vanilla depth); see
// dof_shaderpack.fsh for the shader-pack-depth (~0 near) variant.

...
public final class PhotoPanelScreen extends Screen {
	...
	public void renderBackground(GuiGraphics graphics, int mouseX, int mouseY, float a) {
		// Deliberately empty: keep the live scene visible (no dim, no 1.21 menu blur).
	}
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float a) {
		// Deliberately skip super.render(): it draws the dimming/blurred background.
		Font font = this.font;
		...
		graphics.drawString(font, RESET_ALL_LABEL, ralx, top + 5, resetAllHot ? ACCENT : TAB_DIM, false);
		...
		boolean resetPageHot = mouseX >= rplx - 4 && mouseX < ralx - 8
				&& mouseY >= top && mouseY < top + TABBAR_H;
		graphics.drawString(font, RESET_PAGE_LABEL, rplx, top + 5, resetPageHot ? ACCENT : TAB_DIM, false);
